...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, fig.align = 'center', dev = 'svg', out.height = 6)
library(tidyverse)
...
```{r}
scale <- read_delim("metric | ordinal | nominal
response time, temperature, height, weight, counts | placing in a race, ratings | political party affiliation, face of a rolled dice/coin", delim = "|")
knitr::kable(scale)
```
...
- For example, participants in a footrace can be measured by the time they took to run the race, or by their placing in the race (first, second third, etc), or by the name of the team they represent. 
- These three measurements represent metric, ordinal, and nominal scales, respectively


- siit edasi lineaarne regressioon ja tavalisemad testid, lähtuvalt skaalast
- katse planeerimine ja statistiline jõud
